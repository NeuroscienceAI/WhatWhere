...
def compute_nmi_r2(X, umap_df, mask, N_class):
    temp_emb = umap_df.loc[mask, ["umap1", "umap2"]]
    labels_true = np.array(umap_df.loc[mask, "angle"] * 6).astype(int)
    # --- KMeans for NMI ---
    ...
    return nmi_score, r2_LR, pr, pr_corrected, d_expvar
